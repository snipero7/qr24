...
const PrintButton = dynamic(() => import("@/components/PrintButton").then(m=>m.PrintButton), { ssr: false });
    ...
    <div className="max-w-3xl mx-auto p-8 print:p-0">
      <div className="flex items-start justify-between mb-6 border-b border-black/10 pb-4">
        <div>
          ...
          <div className="text-sm text-gray-600">{L('كود الطلب', 'Order Code')}: <span className="font-mono">{o.code}</span></div>
        </div>
        <img src={dataUrl} alt="QR" className="w-32 h-32" />
      </div>
      <div className="grid grid-cols-2 gap-4 mb-6">
        <Info label={L('العميل', 'Customer')} value={`${o.customer.name} (${o.customer.phone})`} />
        ...
        <Info label={L('السعر', 'Price')} value={String(o.originalPrice)} />
        {(o as any).paymentMethod && (
          <Info label={L('وسيلة الدفع', 'Payment')} value={(o as any).paymentMethod === 'CASH' ? L('نقدًا','Cash') : L('تحويل','Transfer')} />
        )}
        {o.collectedPrice && (
          <>
            <Info label={L('الخصم', 'Discount')} value={`${Math.max(0, Number(o.originalPrice) - Number(o.collectedPrice)).toFixed(2)} ر.س`} />
            <Info label={L('المبلغ بعد الخصم', 'Amount Paid')} value={String(o.collectedPrice)} />
          </>
        )}
